...
<?php } // JavaScript changes this form! See comments.js ?>
    <form name='comment_form' action='<?php echo $h->url(array('page' => $h->post->id)); ?>' method='post' onsubmit="document.getElementById('comment_submit_<?php echo $h->comment->id; ?>').disabled = true; return true;">
        <textarea class="post_comment" name="comment_content" id="comment_content_<?php echo $h->comment->id; ?>" style="width:102%"></textarea><br />
        <div class="comment_subscribe"><input id="comment_subscribe" name="comment_subscribe" type="checkbox" <?php echo $h->vars['subscribe']; ?> /> <?php echo $h->lang['comments_form_subscribe']; ?><?php if ($h->vars['subscribe']) { echo " <small>(" . $h->lang['comments_form_unsubscribe'] . ")</small>"; } ?></div><div style="margin-bottom:6px;"></div>
        ...
        <input type="submit" name="submit" id="comment_submit_<?php echo $h->comment->id; ?>" value="<?php echo $h->lang['comments_form_submit']; ?>" class="submit commentajax" />
        <input type="hidden" id="c_process" name="comment_process" id="comment_process_<?php echo $h->comment->id; ?>" value="newcomment" />
        <input type="hidden" id="c_parent" name="comment_parent" value="<?php echo $h->comment->id; ?>" />
        <input type="hidden" id="c_post_id" name="comment_post_id" value="<?php echo $h->post->id; ?>" />
        <input type="hidden" id="c_user_id" name="comment_user_id" value="<?php echo $h->currentUser->id; ?>" />
